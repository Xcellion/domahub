...
//<editor-fold>-------------------------------PERMANENT-------------------------------------

  //<editor-fold>-------------------------------EMAIL VIEWER-------------------------------------
...
//<editor-fold>-------------------------------COLD EMAIL PARKED DOMAIN OWNERS-------------------------------------

  //<editor-fold>-------------------------------VARIABLES / HELPERS-------------------------------------
  ...
  var sendgrid_marketing_campaign_key = "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";
  ...
  //<editor-fold>-------------------------------GET EMAIL / WHOIS / PARKED STATUS-------------------------------------

  //promise to get list of trending domains from domainscope (filter by keyword + date)
  function get_domainscope_domains(word, index, total, domains_seen, emails_seen, date_to_get){
    return function(){
      return Q.Promise(function(resolve, reject, notify){
        console.log("-----------------------------------Word #" + (index + 1) + "/" + total + " (" + word + ") - " + date_to_get);
        request({
          url: "https://domainscope.com/domainview/trending?keywords=" + word + "&startDate=" + date_to_get + "&endDate=" + date_to_get,
          method: "GET",
          json : true
        }, function(err, response, body){
          buildWhoIsPromises(domains_seen, emails_seen, resolve, reject, body.domains, word, false, date_to_get);
        });
      });
    }
  }
  ...
  function buildWhoIsPromises(domains_seen, emails_seen, resolve, reject, domains, word, property_name, date_to_get){
    ...
    if (emails_for_today){
      ...
      (async () => {
        ...
        for (var x = 0 ; x < emails_for_today.length ; x++){
          if (emails_for_today[x].parked && !emails_for_today[x].current_screenshot && !emails_for_today[x].domahub_screenshot && (
            ...
            screenshot_promises.push(take_screenshot_promise(headless_page, date_to_get, temp_domain_name, x, false));
            screenshot_promises.push(take_screenshot_promise(headless_page, date_to_get, temp_domain_name, x, true));
          }
        }
        ...
        Q.allSettled(screenshot_promises.map(limit(function(item, index, collection){
          ...
          for (var y = 0; y < results.length; y++){
            if (results[y].state == "fulfilled"){
              if (results[y].value.domahub){
                emails_for_today[results[y].value.file_index].domahub_screenshot = results[y].value.imgur_link;
              }
              else {
                emails_for_today[results[y].value.file_index].current_screenshot = results[y].value.imgur_link;
              }
            }
          }
  ...
  function take_screenshot_promise(headless_page, date, domain_name, file_index, domahub){
    return function(){
      return Q.Promise(function(resolve, reject, notify){
        ...
        var http_url = (domahub) ? "https://domahub.com/listing/" + domain_name + "?compare=true&theme=Stars&tutorial=false" : "http://" + domain_name;
        ...
        (async () => {
          ...
          await headless_page.screenshot(screenshot_properties).then(function(buffer){
            ...
            request.post({
              ...
              if (!err && body.success){
                console.log("\x1b[44m%s\x1b[0m" ,"UPLOADED!!! - " + domain_name + " - " + body.data.link);
                resolve({
                  domahub : domahub,
  ...
  function splitAndParseNames(registrant_name, tech_name, admin_name){
    ...
    if (mode_name != "" && mode_name){
      ...
      mode_name = (mode_name.indexOf('@') != -1) ? mode_name.substring(0, mode_name.indexOf('@')) : mode_name;
      mode_name = (mode_name.indexOf('%40') != -1) ? mode_name.substring(0, mode_name.indexOf('%40')) : mode_name;
    ...
    if (emails_for_today){
      for (var x = 0; x < emails_for_today.length ; x++){
        ...
        if (emails_for_today[x].parked && emails_for_today[x].current_screenshot && emails_for_today[x].domahub_screenshot){
          ...
          var split_name = splitAndParseNames(emails_for_today[x].registrant_name, emails_for_today[x].tech_name, emails_for_today[x].admin_name);
          var mode_email = mode([emails_for_today[x].registrant_email, emails_for_today[x].tech_email, emails_for_today[x].admin_email]);

          //if first name is longer than 1 char and we have an email
          if (split_name && split_name.firstName && split_name.firstName.length > 1 && mode_email){
