...
$(document).ready(function() {
	...
	for (var x = 0; x < rentals.length; x++){
		...
		tr.append("<td>" + moment(rentals[x].date).format('YYYY-MM-DD HH:mmA') + "</a></td>");
		tr.append("<td>" + moment(new Date(rentals[x].date).getTime() + rentals[x].duration).format('YYYY-MM-DD HH:mmA') + "</td>");
		tr.append("<td><a href='/listing/" + rentals[x].domain_name + "/" + rentals[x].rental_id + "'>Edit rental</a></td>");
