...
function createDomaintable(){
  ...
  if (listings_to_show.length > 0){
    ...
    for (var x = 0; x < listings_to_show.slice(start_at, start_at + 10).length; x++){
      ...
      if (listings_to_show[start_at + x].min_price){
        clone_row.find(".domain-row-bin").text(moneyFormat.to(listings_to_show[start_at + x].min_price));
      }
      else {
      ...
      if (listings_to_show[start_at + x].price_rate && listings_to_show[start_at + x].price_type && listings_to_show[start_at + x].rentable){
        clone_row.find(".domain-row-bin").text(moneyFormat.to(listings_to_show[start_at + x].price_rate) + " / " + listings_to_show[start_at + x].price_type);
      }
      else {
    ...
    $("#total-domain-count").text("Showing " + listings_to_show.slice(start_at, start_at + 10).length + " of " + listing_info.listings.length + " domains");
    ...
    //show next
    if (start_at + 10 < listing_info.listings.length) {
