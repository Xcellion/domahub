...
function updateDates(tempRow_drop, rental_info){
	for (var x = 0; x < rental_info.date.length; x++){
		...
		var temp_date = tempRow_drop.find("#rental-dates-clone").removeClass('is-hidden').attr('id', "").text(disp_start + " - " + disp_end + " (" + disp_duration + ")");
...
function updateRentalDelete(tempRow_drop, rental_info){
	tempRow_drop.find(".").on("click", function(e) {
